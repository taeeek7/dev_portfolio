...
# 긴급티켓발행자동화 함수 정의 
# 변수항목: 스프레드시트key, json_key, 업로드양식시트명, 파이썬업로드시트명, 슬랙발송채널ID 
def insert_ticket(sheet_key, json_key, ws_name, upload_name, channel_id) :
...
# (기능 추가) 긴급티켓발행자동화_키퍼배정 함수 정의 
# 변수 항목: 스프레드시트key, json_key, 업로드양식시트명, 파이썬업로드시트명, 슬랙발송채널ID 
def ticket_insert_assignment(sheet_key, json_key, ws_name, upload_name, channel_id) :
    ...
    if condition == '발행대기' : 
        ...
        try:
            response_slack = client.chat_postMessage(
                ...
                text=     f"🔖 {branch} 긴급티켓 자동발행\n\n" 
                        + f"   ● 실행일시 : {response_date}\n"
                        + f"   ● 실행건수 : {last_row} 건\n"
                        + f"   ● 결과 : 티켓발행 {success_cnt} 건  /  배정 {assign_cnt} 건  / 실패 {error_cnt} 건\n"
                        + f"         ○ error_roomNo : {error_roomNo}\n"
                        + f"         ○ error_data : {error_data}"
                        )
        except SlackApiError as e:
        ...
        #터미널 결과창 입력
        print(f"파이썬실행_긴급티켓발행_{branch}")
        print(  f"성공 {success_cnt} 건 / 배정 {assign_cnt} 건 / 실패 {error_cnt} 건\n"
            + f"{error_data}")
